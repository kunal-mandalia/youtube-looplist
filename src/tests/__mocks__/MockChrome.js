...
export class MockChrome {
  constructor({ unknownTabs = []}) {
    ...
    const proxyRuntime = new Proxy(this._runtime, {
      get: (obj, prop) => {
        if (prop === 'lastError') {
          const error = obj[prop]
          if (!error) return error

          // if already read / processed return false
          if (this._errorHandling.processedErrors.some(error => error.id === error.id)) {
            return false
          }
          // add to processed errors
          this._errorHandling.processedErrors.push(error)
          return error
        }
        return obj[prop]
      }
    ...
    this.runtime = proxyRuntime

    this.tabs = {
      get: (tabId, cb) => {
        if (this._errorHandling.unknownTabs.includes(tabId)) {
          const error = { id: Date.now(), description: `Unknown tab ${tabId}`}
          proxyRuntime.lastError = error
        }
        cb()
      },
      query: (q, cb) => { cb([{ id: 1 }]) },
